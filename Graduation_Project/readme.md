...
![程序文件显示界面](Doc/images_2020-05-10_23-22-28.png)

9> 对于一些时常会变化的文本显示，比如当前显示的文本名称、当前显示的行号、修改时间或是其他的内容，目前有两种处理办法：
...
1> 参数列表采用坐标的形式标记当前选中控件的高亮操作 [当前行，当前列] （先行后列）

### 网络通信

1> Focas库函数可以实现与数控机床之间的网络通信，根据技术手册可以获取相关的机床信息；

2> 该库函数只支持C++/C#语言，不支持Python语言，可以通过Python程序调用C++代码来解决该问题；

3> 可以根据与机床通信过程的访问数据，自行设计Python版本的库函数；(个人从理论上分析可行)

NOTES：

1> 网络通信应该只能获取到机床信息，不能反向设置机床的相关信息
